function ajax(paramObject) {
    ...
    var promise = new Promise(function (resolve, reject) {
        ...
        xhr.onload = function () {
            if (this.status >= 200 && this.status < 300) {
                // The Ajax request returned a "good" status code, so fulfil the promise.
                resolve(this.response);
            }
            else {
                // The Ajax request returned a "bad" status code, so reject the promise.
        ...
        xhr.onerror = function () {
            // The Ajax request failed for some reason, so reject the promise.
            reject(this.statusText);
        };
    ...
    // Return the promise back to the caller.
    return promise;
}
